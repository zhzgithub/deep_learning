...
# 2.关于权重初始化是否可为0的问题      
	...
	【2.单层感知机】只有一个神经元，他的权重可初始化为0。感知机是误分类驱动。吴恩达根本不讲感知机。所以咱们先不管感
		知机，值学习logistic回归就行了。
	【3.线性回归模型】相当于单层感知机，其权重也可以初始化为0
	【4.logistic回归】其结构相当于感知机，就是单层神经网络，只有一个神经元，权重可初始化为0。
...
这里我很懵逼，没搞懂：
	...
	z一个行向量1*m，输入x是由所有样本横向排列的的矩阵n*m，即每列是一个样本.什么乱七八糟的，z= W.T *x +b的W有转置，dW没转置吗？很懵逼啊。
	
	而讲神经网络的时候，      dW^[2]= dz^[2] * a^[1].T ，比logistic回归多了一个转置符号
							  dW^[1]= dz^[1] * a^[0].T ，这里a^[0]就是所有样本的输入特征x矩阵
	变量z与dz的维度是一致的，w和dw也是相同的维度，z^[1]是n[1]*m矩阵,a^[0]是n[0]*m矩阵，所以需要加上转置符号才能相乘。
	-----------------------------------------------------------------------------------------------
...
# 6.反向传播
	...
	第一层的权重梯度变化：dJ/dW^[1]= (dJ/dz^[1]) * a^[0].T，
								...
								而    dJ/dz^[1]=(dJ/da^[2]) * (da^[2]/dz^[2]) * (dz^[2]/da^[1]) * (da^[1]/dz^[1])
	...
	【疑问】
		1.在前向传播时，把每层得到的Z,W,b缓存起来，当计算反向传播时会用到它们。
		我还没实际操作过，不知道缓存起来怎么用得到，是不是在做更新的时候：
		W=W-α*dW，b=b-α*db用到呢？那也没用到z啊？即使用z对w求偏导，那z也是作为函数，而不是数值，但是吴恩达一直强调缓存Z.

		2.当学习率(步长)α一定时，每次w迭代的梯度都是一样的，即每次w的变化量都是一样的，对不对？
		  我的意思是神经网络结构图中的每一条线代表每一个权值，某一条线的权重每次的变化量都是一定的吗？
		  答：不对。因为每次迭代更新w后w位置变了，成本函数J对w的偏导是一个关于w的函数，因此w在不同位置，dJ/dw的值是不一样的。
		  举个例子：J=w^2，则dJ/dw=2*w,在某个位置比如w=3时，梯度dJ/dw=6，w=w-α*dJ/dw=3-6;
												 而在w=8时，梯度dJ/dw=16,w=w-α*dJ/dw=8-16。
					即在不同位置的w,dJ/dw不一样。所以在不同位置，w的变化量是不一样的。
		
		3.最后预测值和目标值有误差，计算成本函数之后，成本函数是一个值。就是说只要成本函数值不小于给定误差，
		  就一直用成本函数对w求偏导(即梯度)来更新权重W,这时候不能把成本函数看做一个值,而应该是一个关于变量a的函数.对不对？
		  答：对
------------------------------------------------------------------------------------------------------------------
# 7.参数VS超参数
参数parameters指的是w,b等实际参数
超参数hyperparameters指的是能控制实际参数w,b最终结果的一类参数。
hyperparameters如下:
	学习率α
	迭代次数iterations
	隐藏层层数L
...
# 8.logistic回归与softmax的成本函数
 【我发现一个问题】：李航的logistic回归对二分类的求法中分母是1+exp(z),其中一个类别分子是1，另一个类别的分子是exp(z)。
	而softmax对k个类别的输出单元的求法中，分母是∑exp(zj),分子分别为exp(zj),j为输出神经元个数，即类别个数。
	
	讲解logistic回归时，不需要有两个输出单元，logistic回归可看作是单神经元的神经网络。
	讲解softmax时，有几个类别就有几个输出神经单元。样本是什么类型，目标向量的哪个元素位置就是1，其余的元素为0。那么
	预测值是什么样的呢？这几个输出神经元的元素到底都是0和1还是所求出的概率呢？
	并且logistic回归的成本函数与softmax的成本函数的形式不一样。
	logistic成本函数：J(w,b)=-1/m *Σ[y(i)log(a(i))+(1-y(i))log(1-a(i))],即对m个样本求和后再平均，Σ是求和符号  
