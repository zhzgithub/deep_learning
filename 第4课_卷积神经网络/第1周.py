目前我有个问题：卷积神经网络的每个卷积操作的过滤器需要训练吗，是给定过滤器的权重参数呢，还是需要什么规则来训练这过滤器参数。
	视频中讲了全连接层的权重参数是随机初始化的。全连接层后面再接一个softmax层作为输出。
	概念图中，卷积的各层，一层比一层的深度深，最后的一个卷积池化层的图像是个立方体，深度很深，表示最后一个卷积层得到的是提取
	到的很多个特征的图像吧。
...
【7】
	...
	假设输入图像为32*32*3(=3072),过滤器为5*5，共6个，卷积后变为了28*28*6(=4704),如果这两层全连接的话，则有3072*4704=14 milions个参数。
	由于输入图片比较小，所以在现有技术下这个数量的参数还可以接受。但是如果输入图像是1000*1000*3的话就不妙了。
	但是使用卷积层的话，假设一个过滤器为5*5，加上偏置，总共有26个参数，如果有6个过滤器，所以参数共计有156个，这样就少多了。
