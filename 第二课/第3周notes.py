...
【2】正则化网络的激活函数
	...
	所以我们要做的就是计算:
						z~^(i)=γ*z(norm)^(i)+β,					..........(4)
						这里γ和β是模型的
	学习参数。所以我们使用梯度下降或其他一些类似梯度下降的算法，比如Momentum或者Nesterov,Adam，你会更新γ和β，正如更新神经网络
	...
	激活函数，你不想让你的值总是全部集中在靠近0的范围，你想是他们有更大的方差或者不是0均值，以便更好的利用非线性的sigmoid函数，而
	不是使所有的值都集中于这个线性版本中，这就是为什么有了γ和β两个参数后，你可以确保所有的z^(i)值可以是你想赋予的任意值，或者
	它的作用是保证隐藏单元以使均值和方差标准化，那里，均值和方差由两参数控制即γ和β。学习算法可以设置为任何值。所以他的真正作
	用是，使隐藏单元值的均值和方差标准化，即z^[i]有固定的均值和方差，均值和方差可以是0和1，也可以是其他值，它是由γ和β两参数控制的。
