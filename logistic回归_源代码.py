...
【0个显示for循环结构】
  Y=[y(1),y(2),...,y(m)]
  A=[a(1),a(2),...,a(m)]
  Z=w(T)*X+b	#这里的Z是矩阵(行向量),权重w可以不用注重大写还是小写
   =np.dot(w(t),X)+b
   ==[z(1),z(2),...,z(m)]
由dz(1)=a(1)-y(1)可得到：
dZ=[dz(1),dz(2),...,dz(m)]
...
dW=(1/m)*X*dZ(T)
  =(1/m)[x(1),x(2),...,x(m)]*dZ(T)
  =(1/m)[x(1)*dz(1)+x(2)*dz(2)+...+x(m)*dz(m)]

# 梯度更新
W = W-α*dW	#其实就只有，因为不管你大写还是小写，w都为[w1,w2,...wn]的转置，因为w是n维列向量
b = b-α*db	#b是常数，不是向量，因为他是最后用来加一下而已
